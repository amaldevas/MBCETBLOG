  ...
  class BlogController extends CI_Controller 
{
	...
	public function facultyCreate()
	{
		if($this->isLoggedInAdmin())
		{
			if ($this->input->post())
	   	 	{
	   	 		$credentials['userId']=$this->session->userdata('id');
	   	 		$credentials['date'] = date('Y-m-d H:i:s');
	    		$credentials['designation']=$this->input->post('designation');
	    		$credentials['qualification']=$this->input->post('qualification');
	    		$credentials['email']=$this->input->post('email');
	    		$credentials['phoneno']=$this->input->post('phoneno');
	    		$credentials['area']=$this->input->post('area');
	    		$credentials['name']=$this->input->post('name');
					$config['upload_path'] = './assets/faculty/';
					
					$config['allowed_types'] = 'gif|jpg|png';
					$config['max_size']	= '100000';
					$config['max_width']  = '22240';
					$config['max_height']  = '22240';
					
					$this->load->library('upload', $config);
					if ( ! $this->upload->do_upload('faculty-image'))
					{
						$data['error'] = $this->upload->display_errors();
						var_dump($data['error']);
						die;
						redirect('user/faculty-add');
					...
					else
					{
						...
						$credentials['image']= $upload_data['file_name'];
						$this->BlogModel->createFaculty($credentials);
	...
	public function facultyAdd()
	{
		if($this->isLoggedInAdmin())
		{
			$this->load->view('user/addFaculty');
		}
		else
		{
			redirect('user/login');
